...
ONE OUTPUT HEAD, SELECTED BY ``mode``. There is no occurrence-classifier head and no gating: the model emits a
single map whose meaning is fixed by the prepared data's mode, read from ``target_stats['mode']``.
...
WHY THE HOURLY HEAD EMITS A LOGIT RATHER THAN A PROBABILITY. The sigmoid lives in exactly ONE place —
:meth:`DeterministicUnetModule._to_prediction` — so the head can stay in logit space, where Platt scaling composes
as a plain affine term BEFORE the sigmoid (which is what makes it a recalibration of the probabilities). A hourly
regression loss gets that same single sigmoid applied on its way in.
...
training, because clamping a live gradient would silently zero it for every over-predicting cell.
...
# 'train' is always run; at most ONE calibration phase follows it, and which one is a consequence of the mode (a
# monotone recalibration of hours is meaningless on a probability, and Platt scaling is meaningless on hours).
PHASES = ('train', 'occurrence_calibration', 'regression_calibration')
...
class DeterministicUnetModule(L.LightningModule):
    """Deterministic U-net for one sampled trial (see the module docstring for the mode dispatch).
    ...
    Args:
        ...
        in_channels: Number of input feature channels (the dataset's, including the appended ``upstream`` channel in
            residual mode).
        target_stats: Train-split target statistics from the preparation stage. ``mode`` is read from here — it is
            a property of the DATA, not a hyperparameter, so it cannot be a trial key.
    ...
    Raises:
        ValueError: If ``target_stats`` carries no ``mode``; if the normalization channel count disagrees with
            ``in_channels`` (the residual-mode channel check); or if the sampled loss needs an ensemble, which a
            deterministic single forward pass cannot provide.
    """
    ...
    def __init__(self, trial: dict, in_channels: int, target_stats: dict, normalization: dict):
        ...
        self.output_calibration = PlattScaling() if (self.hourly and occurrence_calibration == 'platt') else None
        ...
        # --- network: a single head. classifier_architecture=None builds no second head (unet.py) ------------
        ...
        if self.loss_takes_logits:
            ...
            if binary_loss.needs_ensemble:
                raise ValueError(
                    f'Loss "{loss_name}" needs an ensemble [N, *spatial], but this family is deterministic: one '
                    f'forward pass gives N = 1, so its spread term is identically zero and it degrades silently to '
                    f'a mean absolute error on probabilities. Use it with mc_dropout or diffusion, or pick a '
    ...
    def training_phases(self) -> Tuple[str, ...]:
        """The phase sequence :func:`tuning._fit_trial` fits, in order.

        Always ``train``, followed by at most one calibration phase — and which one is determined by the mode, not
        configured: Platt scaling applies to the hourly logit, the monotone warp to the daily hours."""
        if self.hourly:
            return ('train', 'occurrence_calibration') if self.output_calibration is not None else ('train',)
        return ('train', 'regression_calibration') if self.regression_calibration_enabled else ('train',)

    def set_phase(self, phase: str) -> None:
        """Enter a training phase. A calibration phase freezes the WHOLE backbone and trains only its own layer;
        both calibration layers are identity-initialised and frozen during ``train``, so they are exact no-ops
        until their own phase."""
        if phase not in PHASES:
            raise ValueError(f'Unknown phase "{phase}" (expected one of {PHASES}).')
        if phase == 'occurrence_calibration' and self.output_calibration is None:
            raise ValueError(
        ...
        if phase == 'occurrence_calibration':
            for parameter in self.net.parameters():
                parameter.requires_grad = False
            for parameter in self.output_calibration.parameters():
                parameter.requires_grad = True
            return
        if phase == 'regression_calibration':
            for parameter in self.net.parameters():
                parameter.requires_grad = False
        ...
        # train: the backbone trains, both calibration layers stay frozen at the identity
        for parameter in self.net.parameters():
            parameter.requires_grad = True
        for parameter in self.net.regression_calibration_parameters():
            parameter.requires_grad = False
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        regression_raw, _ = self.net(x)              # classifier_architecture=None -> the second element is None
    ...
    def _head_output(self, raw: torch.Tensor) -> torch.Tensor:
        ...
        frozen outside its own phase, so a no-op during training). Hourly applies Platt scaling, which is affine in
        the logit and therefore sits BEFORE the sigmoid on the prediction path. No sigmoid here: every binary loss
        takes logits and applies its own."""
        if self.hourly:
            return raw if self.output_calibration is None else self.output_calibration(raw)
    ...
    def _to_prediction(self, head_output: torch.Tensor) -> torch.Tensor:
        """Map the head output to the PREDICTION, in the target space, as a detached CPU tensor.
        ...
        head_output = head_output.detach().cpu()
        if self.hourly:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # loss so the monotone map corrects its systematic distortion instead of relearning the identity.
        ...
        elif self.loss_takes_logits:
            loss = self.loss_fn(head, y)                        # head is a LOGIT; the loss sigmoids internally
        else:
            # A distance loss consumes the TARGET SPACE, so hourly hands it the probability -- the one sigmoid,
            # applied here as it is on the prediction path. Daily hands over the activated hours unchanged.
            ...
            loss = self.loss_fn(prediction, y, weights, torch.ones_like(y))
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch[0], batch[1]
        self._val_prediction.append(self._to_prediction(self._head_output(self(x))).float().numpy())
    ...
    def predict_step(self, batch, batch_idx):
        """The shared ensemble contract: ``observation`` and ``prediction`` both ``[B, H, W]``, and NO
        ...
        x, y = batch[0], batch[1]
        prediction = self._to_prediction(self._head_output(self(x))).float()
        return {
