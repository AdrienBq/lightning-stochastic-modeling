...
# =====================================================================================================================
# A synthetic dataset root + split, so the stage can be driven end to end without $DATA_ROOT
# =====================================================================================================================
@pytest.fixture
def dataset_root(tmp_path):
    """A ``$DATA_ROOT``-shaped directory: ``metadata.json``, ``metadata.csv``, ``samples/sample_XXXXXX.pt``.

    The lightning channel is built with a KNOWN per-hour stroke pattern rather than noise, so the target derivation is
    checkable by hand: cell ``(0, 0)`` gets exactly one stroke in 3 hours (sub-threshold at ``hourly_threshold=2``)
    and cell ``(1, 1)`` gets five strokes in 4 hours (qualifying).
    """
    def build(n_days=6, hours=HOURS):
        ...
        for day in range(n_days):
            ...
            lightning[:3, 0, 0] = 1.0                            # single-stroke hours: dropped at threshold 2
            lightning[:4, 1, 1] = 5.0                            # qualifying hours
...
def split_config(tmp_path):
    """A by_sample_id split over the synthetic ids — the same method the smoke tiers use, and the only one that can
    slice below a year."""
    def build(n_days=6):
        import yaml
...
def prepared(dataset_root, split_config, tmp_path):
    """Run the stage and return ``(output_path, prepared_config, target_stats, split_index)``."""
