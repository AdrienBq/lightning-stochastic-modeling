...
| `unet.MonotoneCalibration` | 206 | ~~🔢 `COUNT-REG`~~ Monotone calibration head — works in **log1p space** and needs a non-negative prediction | **keep** (Step 1 Q2, confirmed Step 2): the `0–24` target IS non-negative and, with no transform, is the prediction space, so log1p is valid. Exposed as `calibration.regression` in every search space. See the struck §6 item 9 |
...
| File / symbol | Line | Role | Decision |
|---|---|---|---|
| **`unet.enable_mc_dropout`** | 19 | Forces dropout layers active at inference — **the mechanism the whole family depends on** | keep |
| **`mc_dropout_module.MCDropoutModule`** | 75 | The current two-phase MC-dropout Lightning module | **modify (Step 2 decision).** MC-dropout is now **warm-startable from the deterministic U-net**: when `tune` is given an `upstream-model-path`, the upstream's weights are loaded into an identically-shaped MC-dropout U-net, phase 1 is skipped and only the finetuning phase runs. Unset ⇒ the existing two-phase fit from scratch. The module has `set_phase('train'\|'finetune')` and a `finetuning_enabled` gate but **nothing that loads foreign weights**, so this is NEW code plus a load-time architecture-compatibility check that must RAISE (naming the offending field) rather than silently partial-load. This is why `deterministic_unet`'s search space now fixes `unet.normalization: group` — a batch-norm upstream is unusable for MC inference. ⚠️ Note this is a *weight* initialization, unlike diffusion's residual mode, which conditions on the upstream *prediction* via `prepare_regression` |
| `mc_dropout_module.build_output_activation` | 54 | Output activation factory (`max_hours` clamp) | modify — must read the **`min_hours`/`max_hours` pair** now that both bounds are explicit: `clamped_sigmoid` becomes `min_hours + (max_hours - min_hours) * sigmoid(z)`. Note `softplus` only guarantees `>= 0`, so it silently violates any non-zero `min_hours` — constrain that combination out or clamp after the activation |
...
| 8 | `module.DistrRegressionModule` → **`DeterministicUnetModule`** (renamed in Step 2) | Remove `transform_enabled` / `self.transform`, the `transform.forward(y)` call in the training step, and the `transform.inverse(...)` on the validation/predict path |
| 9 | `unet.MonotoneCalibration` | ~~🔢 Remove (log1p space ⇒ count regressor) — and with it the second `apply_constraints` rule~~ **STRUCK (Step 2).** Superseded by `step-1-design.md` architecture Q2, which resolved this as *"in scope, keep — and extend to MC-dropout"*: with the transform gone the prediction space is the non-negative `0–24` target itself, so log1p is well-defined again and the 🔢 flag no longer applies. What item 6 *does* still remove is the `apply_constraints` rule that disabled the calibrator for the **gaussianized** transform — that transform no longer exists. So the head stays and the search spaces carry `calibration.regression` alongside `calibration.occurrence` |
